...
public interface FlightRepository extends JpaRepository<Flight, Integer> {
	...
	@Query("SELECT f from Flight f inner join f.startDestination sd inner join f.finalDestination fd inner join f.airLine a where a.airLineName like %?9% and sd.destinationName like %?3% and fd.destinationName like %?4% and (?5 is null or f.takeOffDate >= ?5) and (?6 is null or f.landingDate <= ?6) and (?1 is null or f.type = ?1) and (?2 is null or f.flightClass = ?2) and (?7 is null or f.ticketPrice > ?7) and (?8 is null or f.ticketPrice < ?8) and (?10 is null or f.flightLength = ?10)")
