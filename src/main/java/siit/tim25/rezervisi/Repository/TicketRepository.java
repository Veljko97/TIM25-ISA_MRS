...
public interface TicketRepository extends JpaRepository<Ticket, Integer> {
	...
	@Query("SELECT new siit.tim25.rezervisi.DTO.TicketReportDTO(t, SUM (t.ticketPrice), week(t.created)) from Ticket t WHERE t.airLine.airLineID = ?1 GROUP BY week(t.created)")
	public List<TicketReportDTO> getWeeklyReport(Integer airlineId);
	
	@Query("SELECT new siit.tim25.rezervisi.DTO.TicketReportDTO(t, SUM (t.ticketPrice), t.created) from Ticket t WHERE t.airLine.airLineID = ?1 GROUP BY month(t.created)")
	public List<TicketReportDTO> getMonthlyReport(Integer airlineId);
	
	@Query("SELECT new siit.tim25.rezervisi.DTO.TicketReportDTO(t, SUM (t.ticketPrice), t.created) from Ticket t WHERE t.airLine.airLineID = ?1 GROUP BY day(t.created)")
